...
1. Create a dedicated directory on the device: `/sdcard/ToggleTalkModels/` (resides on fast emulated internal flash storage, guaranteeing zero wear-and-tear during read-only model initializations).
...
1. **Create `run_antigravity.sh`** at `/data/data/com.termux/files/home/ToggleTalkAndroid/run_antigravity.sh`:
   ...
   ```
2. **Note on Legacy Scripts & Documentation**:
   - Legacy files (`stt`, `tts`, `toggle_talk_antigravity.sh`, etc.) are retained in the project for reference but are marked as deprecated and unused. Notes will be added to `PROJECT_OVERVIEW.md` and `AGENTS.md` to reflect this.
...
- Update `PROJECT_OVERVIEW.md` and `AGENTS.md` to flag legacy files and document native operations. (Do not touch `PROJECT_PLAN.md` file).
...
To maintain a high level of code integration safety, we will run `git add` and `git commit` to commit changes at the completion of each implementation milestone.
- **Commit 1**: Setup gradle dependency, JNI libraries, and AndroidManifest permissions.
- **Commit 2**: Implement native audio recording and integrated Whisper STT transcription.
- **Commit 3**: Implement new `run_antigravity.sh` Termux wrapper script and hook up its pending intent response callback.
- **Commit 4**: Implement native Kokoro TTS synthesis and AudioTrack playback.
- **Commit 5**: Apply documentation notes to `PROJECT_OVERVIEW.md` and `AGENTS.md`.
